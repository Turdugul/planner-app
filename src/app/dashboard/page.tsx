...
export default function Dashboard() {
  const { user, loading: authLoading } = useAuth();  // Get the user and loading states from useAuth
  const [goals, setGoals] = useState<Goal[]>([]);
  const [tasks, setTasks] = useState<Task[]>([]);
  const [diaryEntries, setDiaryEntries] = useState<DiaryEntry[]>([]);
  const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    if (!user) return; // If user is not authenticated, don't try to fetch data
    ...
    fetchData();
  }, [user]); // Only run fetchData when the user is authenticated
